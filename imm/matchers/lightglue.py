...
class LightGlue(MatcherModel):
    ...
    def _forward(self, data: dict) -> dict:
        ...
        # swap desc to [B x N x D] if needed
        if desc0.shape[2] > desc0.shape[1]:
        ...
        if self.conf.add_scale_ori:
            kpts0 = torch.cat(
                [kpts0] + [data[k].unsqueeze(-1) for k in ("scales", "oris")],
                -1,
            )
            kpts1 = torch.cat(
                [kpts1] + [data[k].unsqueeze(-1) for k in ("scales", "oris")],
                -1,
            )
...
def _make_model(
    ...
    **kwargs: Dict[str, Any],
    ...
    # create model
    model = LightGlue(features=cfg.get("features", None), conf=cfg, **kwargs)
...
@MATCHERS_REGISTRY.register(
    name="lightglue_superpoint",
    default_cfg=default_cfgs["lightglue_superpoint"],
)
def lightglue_superpoint(cfg: Dict[str, Any] = {}, **kwargs):
    return _make_model(name="lightglue_superpoint", cfg=cfg, **kwargs)


@MATCHERS_REGISTRY.register(name="lightglue_disk", default_cfg=default_cfgs["lightglue_disk"])
def lightglue_disk(cfg: Dict[str, Any] = {}, **kwargs):
    return _make_model(name="lightglue_disk", cfg=cfg, **kwargs)


@MATCHERS_REGISTRY.register(name="lightglue_aliked", default_cfg=default_cfgs["lightglue_aliked"])
def lightglue_aliked(cfg: Dict[str, Any] = {}, **kwargs):
    return _make_model(name="lightglue_aliked", cfg=cfg, **kwargs)


@MATCHERS_REGISTRY.register(name="lightglue_sift", default_cfg=default_cfgs["lightglue_sift"])
def lightglue_sift(cfg: Dict[str, Any] = {}, **kwargs):
    return _make_model(name="lightglue_sift", cfg=cfg, **kwargs)
